...
<div class="card card-primary">
     <div class="card-header">
        <h3 class="card-title"><?= isset($transaction) ? 'Edit Hawala Transaction' : 'Add New Hawala Transaction'; ?></h3>
    </div>

    <?= form_open(isset($transaction) ? 'admin/hawala_staff_transaction/edit/' . $transaction->id : 'admin/hawala_staff_transaction/create', ['id' => 'transactionForm']); ?>
        <div class="card-body">
            <div class="row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <select name="staff_id" id="staff_id" class="form-control" required <?= $disable_fields ?>>
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <select name="bank_id" id="bank_id" class="form-control" required <?= $disable_fields ?>>
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="birr" name="birr"
                               value="<?= set_value('birr', isset($transaction) ? number_format($transaction->birr, 2) : ''); ?>" 
                               oninput="formatAndCalculate(this, 'birr')" required >
                    </div>
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="rate" name="rate"
                               value="<?= set_value('rate', isset($transaction) ? number_format($transaction->rate, 2) : ''); ?>"
                               oninput="formatAndCalculate(this, 'rate')">
                    </div>
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="amount" name="amount" readonly
                               value="<?= set_value('amount', isset($transaction) ? number_format($transaction->amount, 2) : ''); ?>">
                    </div>
                </div>
              ...
              <div class="input-group">
                  ...
                  <?php if (!empty($forced_type)): ?>
                      ...
                      <input type="hidden" name="type" id="types" value="<?= $forced_type ?>">
...
$(document).ready(function() {
    ...
    setupValidation('#transactionForm', {
        ...
        rate: {
            number: "Enter a valid exchange rate",
...
/**
 * Format input and trigger calculation
 */
function formatAndCalculate(input, fieldType) {
    // Format the input value
    const formattedValue = formatCurrency(input);
    
    // Trigger appropriate calculation
    if (fieldType === 'birr' || fieldType === 'rate') {
        calculateAmount();
    }
}
...
function calculateAmount() {
    ...
    const birrValue = parseFloat($('#birr').val().replace(/,/g, '')) || 0;
    const rateValue = parseFloat($('#rate').val().replace(/,/g, '')) || 1;
    
    // Calculate the amount
    const calculatedAmount = birrValue / rateValue;
...
function showConfirmationDialog() {
    ...
    const rawBirr = parseFloat($('#birr').val().replace(/,/g, '')) || 0;
    // Get FORMATTED values for display
    const formattedBirr = rawBirr.toLocaleString('en-US', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
    });
    // Get other form values
    const exchangeRate = $('#rate').val();
    const calculatedAmount = $('#amount').val();
            ...
            <div style="font-size: 16px; text-align: left;">
                ...
                <div style="margin-top: 1rem; background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 1rem;">
                    <ul style="list-style: none; padding: 0; margin: 0;">
                        ...
                        <li style="margin-bottom: 0.5rem;">
                            <strong>💵 Amount (Birr):</strong>
                            <span class="swal2-highlight" style="color: #16a34a; font-weight: 600;"> ${formattedBirr} ETB</span>
                        </li>
                        <li style="margin-bottom: 0.5rem;">
                            <strong>🔁 Exchange Rate:</strong>
                            <span class="swal2-highlight">${exchangeRate}</span>
                        </li>
                        <li style="margin-bottom: 0.5rem;">
                            <strong>🧮 Calculated Amount:</strong>
                            <span class="swal2-highlight" style="color: #16a34a; font-weight: 600;">${calculatedAmount} ${Currency} </span>
                        </li>
                        <li style="margin-bottom: 0.5rem;">
                            <strong>🧑‍💼 Staff:</strong>
        ...
        html: `
            <div style="text-align:center;">
                <div class="swal2-content-custom">
                    ...
                    ${calculatedAmount ? `<p>Calculated amount: <strong>${calculatedAmount}  ${Currency}</strong></p>` : ''}
                </div>
            </div>
        `,
        icon: 'success',
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    $.each(data, function(index, bank) {
                        let isSelected = selectedBankId && String(selectedBankId) === String(bank.id) ? 'selected' : '';
                        options += `<option value="${bank.id}" ${isSelected}>
                            ${bank.name} - ${bank.number} (${parseFloat(bank.balance).toFixed(2)} Birr)
                        </option>`;
                    });
...
$(document).ready(function () {
    // Get staff and bank id from PHP for edit mode
    const staffId = $('#staff_id').val();
    const selectedBankId = '<?= isset($transaction) ? $transaction->bank_id : '' ?>';

    // If editing or preloaded staff exists, load their banks
    if (staffId) {
        loadStaffBanks(staffId, selectedBankId);
    }

    // Reload banks when staff selection changes
    $('#staff_id').on('change', function () {
        const newStaffId = $(this).val();
        loadStaffBanks(newStaffId);
    });
});
</script>
